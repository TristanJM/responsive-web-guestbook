...
$(document).ready(function(){
	...
	if (navigator.geolocation) $('.js-geolocation').show(); 
	else $('.js-geolocation').hide();
...
function checkSize() {
	...
	if ($(window).width() <= 480) {
		...
		if ( $('#nav-menu-icon').attr('style') === "display:none;" ) createSmartNav();
	}
	else {
		// If not smallest breakpoint AND smart-nav hasnt already been removed
		if ( $('#nav-menu-icon').attr('style') !== "display:none;" ) removeSmartNav();
	}
}
...
// JavaScript to fix any issues with individual browsers
	...
	if (browser == "Unknown") {
		...
		if ($(window).width() > 780) {
			if ($('#lu-logo-header').attr('src') === "images/lu-shield-white.png") {
				$('#lu-logo-header').attr('src','images/lu-logo-white.png');	// switch to full logo
			}
		}
		else {
			if ($('#lu-logo-header').attr('src') === "images/lu-logo-white.png") {
				$('#lu-logo-header').attr('src','images/lu-shield-white.png');	// switch to shield logo
			}
		}
		// Large hero image
